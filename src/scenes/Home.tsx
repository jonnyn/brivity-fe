...
export default function Home() {
  ...
  const createPostMutation = useMutation(
    (newPost: Post) => createNewPost(state.user?.auth, newPost),
    {
      onSuccess: () => {
    ...
    <div className="flex flex-col">
      <div className="h-full overflow-auto sm:-mx-6 lg:-mx-8">
        <div className="py-2 align-middle inline-block sm:px-6 lg:px-8">
          ...
          <table className="shadow border-b border-gray-200 sm:rounded-lg divide-y divide-gray-200 h-2/3">
            ...
            <tbody className="bg-white divide-y divide-gray-200 h-2/3 overflow-y-scroll">
              {data?.posts?.map((post) => (
                <tr key={post.id}>
                  ...
                  <td className="px-6 py-4 text-right text-sm font-medium flex">
                    ...
                    <button
                      style={{
                        display: state.user.id === post?.user?.id ? "" : "none",
                      }}
                      className="text-indigo-600 hover:text-indigo-900 pr-4"
      ...
      <div className="flex justify-between py-4">
        ...
        <Button
          text="Next Page"
          onClick={() => {
            if (!isPreviousData && hasMorePosts) {
              setPage((prev) => prev + 1);
            }
          }}
          disabled={isPreviousData || !hasMorePosts}
        />
      </div>
    </div>
  );
